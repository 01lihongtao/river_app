...
(function($, owner) {
		...
		if (uploadInterval == null) {
			uploadInterval = setInterval(function() {
				...
				if (intervalFlag == 'true') {
					...
					if (uploadDatas==null || uploadDatas.length == 0) {
						return;
					}
					...
					if (uploadFlag) {
						...
						var file_data_arr = [];
						var arr = [];
						for (file of uploadDatas[uploadIndex].arrImgs) {
							arr.push(file);
						}
	...
	function uploadImg(file_data_arr, uploadIndex, uploadDatas, getUploadCode) {
		...
		// console.log("dictionaryKey--->"+app.getSetting("dictionaryKey"));
		uploadData.outfallArroundType = dictionaryKey[uploadData.outfallArroundType];

		uploadData.dischargeType = dictionaryKey[uploadData.dischargeType];
		uploadData.dischargeSubtype = dictionaryKey[uploadData.dischargeSubtype];
		uploadData.dischargeTypeDescribe = dictionaryKey[uploadData.dischargeTypeDescribe];

		uploadData.riverLevel = dictionaryKey[uploadData.riverLevel];
		uploadData.entryRiverType = dictionaryKey[uploadData.entryRiverType];
		uploadData.outfallType = dictionaryKey[uploadData.outfallType];
		uploadData.sewageSourceType = dictionaryKey[uploadData.sewageSourceType];
		...
		for (item in uploadData) {
			...
			if (item != 'arrImgs' && item != 'arrWaterImgs') {
			...
			if (xhr.readyState == 4) {
				...
				if (xhr.status == 200) {
					...
					if (ret.code == 200) {
						...
						mui.toast('上传成功');
						// 更新缓存的待上传数据
						updateUploadDatas(uploadDatas, uploadIndex);
					} else {
						getUploadCode(500);
						mui.toast("ret.code != 200--->" + ret.msg);
					}
				} else {
					getUploadCode(500);
		...
		if (uploadData.eventType == 1) {
			...
			xhr.open('POST', app.url('saveWaterEvidence'), true);
		}
	...
	/**
	 * 更新缓存的待上传数据
	 * @param {Object} uploadDatas
	 * @param {Object} uploadIndex
	 */
	function updateUploadDatas(uploadDatas, uploadIndex) {
		//1 删除缓存中的数据
