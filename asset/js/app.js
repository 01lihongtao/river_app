...
(function($, owner) {
	...
	owner.APP_CATEGORY_UPDATE = function(category) {
		APP_CATEGORY = category;
	}

	owner.APP_CATEGORY = function() {
		return APP_CATEGORY;
	}
	...
	var APP_LOG = false;
	var APP_URL = true;
	var IPV4 = "http://192.168.3.80:8088/";

	owner.IPV4 = function(ip4) {
		IPV4 = ip4;
	}
	...
	//owner.prefix = "http://info.ubinavi.com.cn:6104/";
	// http://116.228.75.54
	owner.prefixvedio = function(path) {
		if (!path) path = ""
		if (!APP_URL) {
			console.log("--->url=" + IPV4 + path);
			return IPV4 + path;
		} else {
			console.log("--->url=" + "http://116.228.75.54:8090/" + path);
			return "http://116.228.75.54:8090/" + path;
		}
	}
	owner.prefixip = function(path) {
		if (!path) path = ""
		if (!APP_URL) {
			return IPV4 + path
		} else {
			// return app.getSetting("switch_ip") + ":8081/" + path
			return app.getSetting("switch_ip")  + path
		}
	}
	owner.prefix = function(path) {
		if (!path) path = ""
		if (!APP_URL) {
			return IPV4 + path
		} else {
			return app.getSetting("switch_ip") + ":8081/api/" + path;
	...
	owner.url = function(path) {
		if (!APP_URL) {
			console.log("--->url=" + IPV4 + path);
			return IPV4 + "/app/" + path
		} else {
			// console.log("--->url=" + owner.prefix() + "/app/" + path);
			return owner.prefix() + "/app/" + path;
		} 
	}
	owner.url1 = function(path) {
		console.log("--->url=" + owner.prefix() + path);
		return owner.prefix() + path;
	}

	owner.url2 = function(path) {
		if (!APP_URL) {
			console.log("--->url=" + IPV4 + path);
			return IPV4 + path
		} else {
			console.log("--->url=" + owner.getSetting("switch_ip") + path);
			return owner.getSetting("switch_ip") + path;
		}
	}
 ...
 //   owner.getData = function (url, params) {
	//     let paramsArray = [];
	//     //拼接参数
	...
	owner.httpGet = function(_url, params,callback) {
		// Object.keys(params).forEach(key => paramsArray.push(key + '=' + params[key]))
		// if (_url.search(/\?/) === -1) {
		...
		// console.log('真实Url'+_url)
		
		mui.ajax(_url+'?user=admin&password=827ccb0eea8a706c4c34a16891f84e7b', {
	 ...
	 * yyyy-MM-dd HH:mm:ss
	 */
	owner.getFormatDayTime = function() {
		var date = new Date();
